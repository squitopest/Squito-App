      ...
      {isDrawerOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-[9600] flex max-h-[85vh] flex-col rounded-t-[28px] border-t border-white/10 bg-squito-cardDark/95 backdrop-blur-2xl shadow-[0_-8px_30px_rgba(0,0,0,0.3)]"
          >
            {/* Handle + Header */}
            <div className="flex flex-col items-center px-5 pt-3 pb-2">
              ...
              <div className="flex w-full items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {hasItems && (
                    <span className="rounded-full bg-squito-green/15 px-2.5 py-0.5 text-xs font-bold text-squito-green">
            ...
            {!hasItems && (
              <div className="flex flex-1 flex-col items-center justify-center px-8 py-12 text-center">
                ...
                <p className="mt-2 text-base font-medium text-white/40 max-w-xs">
                  Browse our services and add the ones you need. Book them all at once!
                </p>
                <Link href="/plans" onClick={() => { closeDrawer(); haptics.light(); }}>
                  <GlassButton
                    variant="primary"
                    className="mt-6 bg-squito-green/90 dark:bg-squito-green px-8 py-3 text-md"
                  >
                    Browse Services
                  </GlassButton>
                </Link>
            ...
            {hasItems && (
              <>
                  ...
                  {items.map((item, idx) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-md font-bold text-white leading-snug truncate">
                          {item.serviceName}
                        </p>
                        <p className="text-xs font-medium text-white/40 mt-0.5 truncate">
                          {item.description}
                        </p>
                        {item.points > 0 && (
                          <span className="mt-1 inline-flex items-center gap-1 text-2xs font-bold text-squito-green">
                            ⭐ +{item.points} pts
                          </span>
                        )}
                      </div>
                ...
                <div className="border-t border-white/10 bg-white/5 px-5 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
                  ...
                  <p className="text-2xs text-white/20 mb-3">
                  ...
                  <button
                    ...
                    className="mt-3 w-full text-center text-sm font-bold text-red-400 active:opacity-70 transition-opacity"
                  >
                    Clear Cart
                  </button>
                </div>
